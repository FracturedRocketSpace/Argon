...
def plotResults(particles, temp, eK, eP, pressure, Cv, displacement):
    ...
    plt.ylabel(r'Temperature $\left(\frac{\epsilon}{k_b}\right)$')
    ...
    plt.title('Pressure')
    plt.xlabel(r'Time ($\tau$)')
    plt.ylabel(r'Compressibility factor $\left(\frac{V}{\sigma^3}\right)$')
    plt.plot(timeAxis, pressure)
    
    plt.subplot(235)
    plt.title('Cv')
    plt.xlabel(r'Time ($\tau$)')
    plt.ylabel(r'Cv $\left(\frac{k_b}{V}\right)$')
